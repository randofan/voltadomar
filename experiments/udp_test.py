...
def build_udp_probe(destination_ip, ttl, source_port, dst_port):
    ...
    '''
    ip_packet = IP(dst=destination_ip, ttl=ttl, proto=17, ecn=False)
    udp_packet = UDP(sport=source_port, dport=dst_port, checksum=123)
...
async def receiver_worker(icmp_socket):
    ...
    while time.time() < end:
        ...
        dst_port = udp.dport
        ecn = ip.ecn
        print(f"Received ICMP packet at {recv_time} with IP src: {ip_src}, ECN: {ecn}, ICMP type: {icmp_type}, UDP sport: {src_port}, UDP dport: {dst_port}")
